...
// The public storefront (Phase 4): a client-routed SPA at /store, same
// deep-link fallback shape as the Guide (mirrored in server/main.ts serveStatic).
function isStoreSpaPath(pathOnly: string): boolean {
  if (pathOnly !== '/store' && !pathOnly.startsWith('/store/')) return false;
  const last = pathOnly.slice(pathOnly.lastIndexOf('/') + 1);
  return !last.includes('.');
}
function staticPageAliasPlugin() {
  const rewrite = (req: { url?: string }) => {
    ...
    const target =
      STATIC_PAGE_ALIASES.get(pathOnly) ??
      (isGuideSpaPath(pathOnly)
        ? '/guide.html'
        : isStoreSpaPath(pathOnly)
          ? '/store.html'
          : undefined);
