...
export async function ensureSchema(): Promise<void> {
  ...
  try {
    ...
    // Shop catalog (Phase 1 backend foundation): categories, then products
    // (category_id references shop_categories), then inventory (product_id
    // references shop_products). Applied unconditionally (idempotent), like
    // the other schema modules.
    ...
    // DO NOTHING), same advisory lock, additive only (never touches a row an
