 ...
 *  - the shop catalog admin surface (server/shop_categories_routes.ts,
 *    shop_products_routes.ts, shop_inventory_routes.ts, shop_orders_routes.ts:
 *    admin-only CRUD for categories/products/inventory/orders behind
 *    requireAdmin, the same instance every other admin route mounts;
 *    registry-only RouteDefs born after the migration, same new-route rule as
 *    the deeds and Steam families, so they carry no legacy ladder twin);
