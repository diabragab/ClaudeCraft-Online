...
export const ERROR_CODES = deepFreeze({
  ...
  // shop: the admin catalog CRUD surface (categories/products/inventory).
  // One shared pair across all three domains, mirroring the new:endpoint
  // scaffold's own <domain>.not_found / <domain>.invalid_input convention.
  // The admin envelope (serializeAdmin) never spreads HttpError params into
  // the response body, so both codes carry none; the specific domain-rule
  // reason (e.g. 'no_price', 'parent_cycle') is server-side detail only.
  'shop.not_found': { params: [] },
  'shop.invalid_input': { params: [] },
  // shop: order-specific rejections (Phase 3, orders). Distinct from the
  // generic invalid_input pair above because the admin UI reacts to them
