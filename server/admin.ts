...
// Exported so every admin-surface route (including sibling domain modules like
// server/shop_categories_routes.ts) mounts this SAME gate instance rather than
// building a parallel one: tests/server/http/ownership_coverage.test.ts drives
// every operator-scoped route through the one setAdminDbForTests seam, so a
// second independent gate would silently fall through to the real db in that
// sweep (and in production would just be needless duplication of this exact
// logic).
