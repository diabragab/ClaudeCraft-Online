...
const ADMIN_ERROR_KEYS: Record<string, string> = {
  ...
  // The shop catalog routes (server/shop_categories_routes.ts and siblings) are
  // the first admin routes to answer with a stable machine CODE rather than
  // legacy English prose (server/http/error_codes.ts); these four are the ones
  // an operator can actually hit. Codes are already lowercase, so they key this
  // map identically to the prose rows above.
  'shop.not_found': 'error.shopNotFound',
  'shop.invalid_input': 'error.shopInvalidInput',
  'shop.out_of_stock': 'error.shopOutOfStock',
  'shop.invalid_status_transition': 'error.shopInvalidStatusTransition',
  'validation.failed': 'error.validationFailed',
  'db.conflict': 'error.dbConflict',
};
