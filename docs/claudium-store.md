...
## In-game purchase path (Phase 5 of the Shop System)

The Treasure Chest icon's WOC Store Store tab no longer buys weapon skins through a bespoke
`kind: "skin"` spend call directly against this catalog. Every Season 1 skin is now ALSO a
`shop_products` row (`grantKind: 'weapon_skin'`, seeded from `WEAPON_SKIN_LIST` by
`server/shop_armory_seed.ts`), and the in-game purchase flow goes through the Shop System's
Claudium checkout (`POST /api/shop/claudium/purchase`, see `API.md` and `SHOP_SYSTEM.md`'s
"Phase 5 scope decisions"). That checkout still calls `claudiumSpend` below with the identical
`kind: 'skin'` / `itemId: <skin id>` shape, so this document's balance/price/catalog authority
split is unchanged; only what CALLS `claudiumSpend` moved from `server/claudium.ts`'s old
`/api/claudium/spend` route to `server/shop_claudium_checkout.ts`.
