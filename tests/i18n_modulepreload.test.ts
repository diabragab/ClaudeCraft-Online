...
describe('i18n modulepreload build hook', () => {
  ...
  describe('localeChunkMap', () => {
    ...
    it('honors a non-root base', () => {
      expect(localeChunkMap(manifest, ['es'], '/app/')).toEqual({
        es: '/app/assets/es-aaaa1111.js',
      });
    });
  ...
  describe('templateModulepreload (FS orchestrator the Vite writeBundle plugin hook calls)', () => {
    it('resolves the manifest + loaders source and rewrites dist/index.html in place', () => {
      ...
      try {
        ...
        writeFileSync(
          path.join(genDir, 'loaders.ts'),
          "export const SUPPORTED_LANGUAGES = ['en', 'es', 'de_DE'] as const;\n",
        );
        writeFileSync(
        ...
        writeFileSync(
          ...
          `<head><script>var m = ${PLACEHOLDER};</script></head>`,
        );

        const { map, htmlPath, manifestPath } = templateModulepreload({
          root: tmp,
          outDir,
          base: '/',
        });
        ...
        expect(rewritten).toContain(
          '{"es":"/assets/es-aaaa1111.js","de_DE":"/assets/de_DE-bbbb2222.js"}',
        );
      } finally {
    ...
    it('propagates the fail-closed throw when a locale has no manifest chunk', () => {
      ...
      try {
        ...
        writeFileSync(
          path.join(genDir, 'loaders.ts'),
          "export const SUPPORTED_LANGUAGES = ['en', 'es', 'fr_FR'] as const;\n",
        );
