...
export const SURFACE_INVENTORY: readonly SurfaceRoute[] = [
  ...
  // Public storefront (Phase 4): anonymous, active-only catalog browsing plus
  // the player-scoped "my orders" surface. Same registry-only shape as the
  // deeds/Steam families above: no legacy ladder arm. The catalog routes are
  // rate-limited the same way the deeds rarity read and the character-sheet/
  // search routes are (publicReadRateLimited, in-handler); the order routes
  // are requireAccount-gated (never anonymous) and always scope to the
  // caller's own accountId.
  {
    dispatcher: DISPATCH.mainApi,
    method: 'GET',
  ...
  // requireShopAdmin only accepts GET/POST (like requireAdmin), so update is
  // POST to the plain :id path and delete is a literal POST .../:id/delete
  // suffix rather than the PUT/DELETE methods.
  {
    dispatcher: DISPATCH.admin,
    method: 'GET',
  ...
  // Shop orders (Phase 3): admin-only back-office order entry + fulfillment
  // over the catalog above. Same registry-only shape as categories/products/
  // inventory: no legacy handleAdminApi arm, POST-only writes (status/cancel/
  // refund are literal action suffixes, not PUT/DELETE).
  {
    dispatcher: DISPATCH.admin,
    method: 'GET',
