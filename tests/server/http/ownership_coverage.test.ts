...
// Install the storefront-orders deny environment: the requireAccount guard
// (server/http/middleware/require_account.ts) resolves the valid token to the
// caller via the route's own swappable authDb seam (there is no real Postgres
// pool in this suite), and the order service's getOrder always returns null
// (the requireOwnedOrder loader's account-scoped load then misses regardless
// of what it compares, exercising the deny path GET /api/shop/orders/:id must
// answer with a 404 before its handler runs).
function installDenyingStorefrontOrders(): void {
  setStorefrontOrdersAuthDbForTests({
    accountAndScopeForToken: async () => ({ accountId: CALLER_ACCOUNT_ID, scope: 'full' as const }),
    moderationStatusForAccount: async () => NOT_LOCKED,
  });
