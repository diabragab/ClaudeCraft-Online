...
describe('array', () => {
  it('decodes an array of item-shaped elements, coercing as it goes', () => {
    const s = array(num({ int: true, min: 1 }));
    expect(s.decode(['1', '2', '3'])).toEqual({ ok: true, value: [1, 2, 3] });
  });

  it('rejects a non-array with code type at the array pointer', () => {
    expect(issuesOf(array(num()).decode('nope'))[0]).toMatchObject({ pointer: '', code: 'type' });
    expect(issuesOf(array(num()).decode({}))[0].code).toBe('type');
    expect(issuesOf(array(num()).decode(null))[0].code).toBe('type');
  });

  it('nests each element pointer as /<index>', () => {
    const s = array(object({ quantity: num({ int: true, min: 1 }) }));
    const issues = issuesOf(s.decode([{ quantity: 1 }, { quantity: 'x' }]));
    expect(issues[0]).toMatchObject({ pointer: '/1/quantity', code: 'type' });
  });

  it('collects EVERY element issue in one pass, never first-fail', () => {
    const s = array(num({ int: true, min: 1 }));
    const issues = issuesOf(s.decode(['x', '0', 'y']));
    expect(issues.map((i) => i.pointer)).toEqual(['/0', '/1', '/2']);
  });

  it('enforces minLength and maxLength on the array itself, with params', () => {
    const s = array(num(), { minLength: 1, maxLength: 2 });
    expect(issuesOf(s.decode([]))[0]).toMatchObject({
      code: 'minLength',
      params: { minLength: 1 },
    });
    expect(issuesOf(s.decode([1, 2, 3]))[0]).toMatchObject({
      code: 'maxLength',
      params: { maxLength: 2 },
    });
    expect(s.decode([1, 2])).toEqual({ ok: true, value: [1, 2] });
  });

  it('an empty array is valid when no minLength is set', () => {
    expect(array(num()).decode([])).toEqual({ ok: true, value: [] });
  });

  it('works inside object() as a declared field', () => {
    const s = object({ items: array(num({ int: true, min: 1 }), { minLength: 1 }) });
    expect(s.decode({ items: ['1', '2'] })).toEqual({ ok: true, value: { items: [1, 2] } });
    expect(issuesOf(s.decode({ items: [] }))[0]).toMatchObject({
      pointer: '/items',
      code: 'minLength',
    });
  });
...
// array() infers T[] and composes with object() for a nested array field.
const arraySchema = object({ items: array(object({ quantity: num({ int: true, min: 1 }) })) });
type ExpectedArray = { items: { quantity: number }[] };
type _ArrayInferIsExact = Expect<Equal<Infer<typeof arraySchema>, ExpectedArray>>;
