...
describe('registry completeness: migrated baseline (public reads + auth + characters + account + wallet + reports/telemetry + discord)', () => {
  ...
  const MIGRATED_ROUTES: readonly LadderRoute[] = [
    ...
    // Public storefront (Phase 4, server/shop_storefront_catalog_routes.ts +
    // shop_storefront_orders_routes.ts): registry-only like the deeds/Steam
    // families above, no legacy ladder twin.
    ...
    // Phase 8: the caller's own purchase status poll, keyed on the Stripe
